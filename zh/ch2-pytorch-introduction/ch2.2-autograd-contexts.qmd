...
此时 `y.requires_grad` 是 `True` 还是 `False`？为什么？

### 练习 2｜原理分析题

为什么模型训练阶段通常需要记录计算图，而验证和推理阶段通常会关闭梯度记录？

### 练习 3｜实践题

运行下面的代码，并解释两个输出为什么不同：
...
虽然 `x.requires_grad=True`，但 `y` 的计算发生在 `torch.no_grad()` 上下文中，因此这次计算不会被 Autograd 记录，得到的 `y` 也不会继续跟踪梯度。

### 练习 2｜原理分析题

训练阶段需要通过反向传播计算模型参数的梯度，因此前向计算需要记录相应的计算图。

验证和推理阶段通常只需要得到模型的前向计算结果，并不会进行反向传播或更新参数，因此可以关闭梯度记录。这样可以避免保存反向传播所需的中间信息，减少额外的内存占用和计算开销。
...
外层的 `torch.no_grad()` 暂时关闭了梯度记录，因此 `y = x ** 2` 不会被 Autograd 追踪，`y.requires_grad` 为 `False`。

在内层使用 `torch.enable_grad()` 后，梯度记录被局部重新开启，因此 `z = x ** 3` 会重新被 Autograd 追踪，`z.requires_grad` 为 `True`。

这个例子说明 PyTorch 的梯度模式可以嵌套使用：进入新的梯度上下文时会临时切换模式，退出后再恢复之前的梯度模式。